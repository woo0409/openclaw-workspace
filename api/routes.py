...
async def save_email_settings(
    ...
    try:
        # 这里可以扩展为保存到数据库
        # 目前先使用环境变量或配置文件
        logger.info(f"保存邮箱设置: {email}, enabled: {enabled}")

        # 更新环境变量（仅当前会话有效）
        os.environ["NOTIFICATION_EMAIL"] = email
        os.environ["NOTIFICATION_ENABLED"] = str(enabled).lower()
...
@router.post("/settings/email/daily")
async def send_daily_notification(db: Session = Depends(get_db)):
    """
    发送每日通知邮件
    ...
    try:
        ...
        if not notification_email or not enabled:
            logger.info("邮箱通知未启用或未配置邮箱")
            return {
                "success": False,
                "message": "邮箱通知未启用"
            }
        ...
        if success:
            logger.info(f"每日通知已发送至: {notification_email}")
            return {
                "success": True,
                "message": "每日通知已发送"
            }
